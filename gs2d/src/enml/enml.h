...
Things you must remember:
	-Use '\\' and '\;' to write the '\' and ';' characters to attribute files
	-Use '/' for comments

Things it can't do:
	-Can't have an entity inside another entity. All entities MUST be global.
	-Values that start with a return character '\n'
	...
	*/
	unsigned int parseString(const str_type::string &str)
	{
		...
		for (unsigned int cursor=0; cursor<size; cursor++)
		{
			...
			} else if (status != enmlssREAD_VALUE && status != enmlssCOMMENT && str[cursor] == GS_L('/'))
			{
				lastStatus = status;
