...
void GLES2ShaderContext::DrawRect() const
{
	m_currentVS->SetConstant("depth", m_spriteDepthValue);
...
void GLES2ShaderContext::SetShader(GLES2ShaderPtr vs, GLES2ShaderPtr ps, const math::Matrix4x4 &ortho, const math::Vector2& screenSize)
{
	...
	if (m_currentVS && m_currentPS)
	{
		...
		ResetViewConstants(ortho, screenSize);
...
{
	std::map<std::size_t, GLES2UniformParameterPtr>::iterator iter = parameters.find(fastHash(name));
	...
	{
		parameters[fastHash(name)] = param;
	}
}
